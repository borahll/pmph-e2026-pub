...
let primesFlat (n: i64) : []i64 =
  ...
  let (sq_primes, _) =
    loop (sq_primes, len) while len < n do
      ...
      -- get the start points of each prime
      -- get the end points
      -- scatter the primes to their end position
      let ends = scan (+) 0 mult_lens
      let starts = map2 (-) ends mult_lens 
      let insert_start = scatter (replicate flat_size 0i64)
                                  starts
                                  sq_primes
      let flat_prime = scan (\z p -> if p > z then p else z) 0i64 insert_start
      let segment_end = scatter (replicate flat_size 0i64)
                                starts
                                starts
      let segment_starts = scan (\z p -> if p > z then p else z) 0i64 segment_end
      let not_primes = map (\i ->
              let prime = flat_prime[i]
              let start_of_segment = segment_starts[i]
              in (i - start_of_segment + 2) * prime)
            (iota flat_size)
      --------------------------------------------------------------
